...
export default function PassengerDashboard({
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "bus":
        const nextBuses = mockTrips
          .filter((t) => t.status === "boarding" || t.status === "arriving")
          .slice(0, 5);
        return (
          <div className="p-4 space-y-4">
            <div className="space-y-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Live departure times at {selectedTerminal}
              </p>
            </div>
            ...
            {nextBuses.map((trip) => {
              ...
              return (
                <Card
                  key={trip.id}
                  className={cn("transition-colors", statusConfig.bg)}
                >
                  <CardContent className="p-3">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Bus #{trip.bus_id} • Gate {trip.gate}
                        </p>
                      </div>
                    ...
                    <div className="flex items-center gap-2 text-xs">
                      ...
                      <span>
                        {trip.scheduled_time.toLocaleTimeString("en-US", {
                          hour: "2-digit",
                          minute: "2-digit",
                        })}
                      </span>
      ...
      case "train":
        ...
        const crowdLevel = station
          ? getCrowdLevel(station.currentCrowd, station.capacity)
          : "low";
        const crowdConfig = {
          low: {
        ...
        const stationsByLine = mockStations.reduce(
          (acc, s) => {
          ...
          <div className="p-4 space-y-4">
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  {station?.status === "operational" ? (
                    <Badge
                      variant="outline"
                      className="text-green-600 w-full justify-center"
                    >
                      Operational
                    </Badge>
                  ) : (
                    <Badge
                      variant="outline"
                      className="text-orange-600 w-full justify-center"
                    >
                      {station?.status}
                    </Badge>
                  )}
            ...
            <div>
              ...
              {lines.map((line) => {
                ...
                const displayStations = isExpanded
                  ? lineStations
                  : lineStations.slice(0, 2);
