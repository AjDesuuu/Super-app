...
export default function PassengerRouteView({
  ...
  const getStepTrafficLevel = (stepIndex: number): TrafficColors => {
    ...
    if (hc === TRAFFIC_HEX_COLORS.MODERATE.toLowerCase()) level = "moderate";
    else if (hc === TRAFFIC_HEX_COLORS.HIGH.toLowerCase()) level = "high";
    else if (hc === TRAFFIC_HEX_COLORS.SEVERE.toLowerCase()) level = "severe";
  ...
  useEffect(() => {
    const fetchPOIs = async () => {
      ...
      console.log("Fetching POIs for passenger coordinates:", passengerCoords);
      setLoadingPOIs(true);
      try {
        ...
        const nearbyFiltered = pois.filter((poi) => (poi.distance || 0) <= 200);

        // Randomly select 1-2 establishments once from filtered list
        const count = Math.floor(Math.random() * 2) + 1; // 1 or 2
        const shuffled = [...nearbyFiltered].sort(() => 0.5 - Math.random());
  ...
  }, [currentStepIndex, consolidatedSteps.length, startLocation?.coordinates]);
  ...
  const handleGo = useCallback(() => {
    const nextStep = Math.min(consolidatedSteps.length, currentStepIndex + 1);
          ...
          <div className="px-4 py-3 flex items-center gap-3">
            ...
            <div className="flex-1">
              <h1 className="text-xl font-bold text-blue-600">
                {showingEstablishments ? "Nearby Places" : "Route Steps"}
              </h1>
                        ...
                        <div key={type}>
                          <div className="flex items-center gap-2 mb-2">
                            <span className="text-lg">
                              {POI_CATEGORIES[type]?.icon || "📍"}
                            </span>
                            <h3 className="text-sm font-semibold text-blue-600">
                              {POI_CATEGORIES[type]?.label || type}
                            </h3>
                          ...
                          <div className="space-y-2">
                            {pois.map((poi) => (
                              <Card key={poi.id} className="border-muted">
                                <CardContent className="p-3">
                                  <div className="flex items-start gap-3">
                                    <span className="text-xl">
                                      {POI_CATEGORIES[poi.type]?.icon || "📍"}
                                    </span>
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start gap-3">
                        <div className="flex-shrink-0">
                          <div
                            className={`h-10 w-10 rounded-full ${
                              isCurrentStep ? "bg-blue-600/20" : "bg-muted"
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-2">
                            <Badge
                              variant={isCurrentStep ? "default" : "secondary"}
                            >
                              Step {idx + 1}
                            </Badge>
                            ...
                            {step.mode !== "BusTerminal" &&
                              (() => {
                                const mode = getConsolidatedMode(step);
                                const isLRTMRT =
                                  mode.vehicle.includes("LRT") ||
                                  mode.vehicle.includes("MRT");

                                if (
                                  isLRTMRT &&
                                  step.previousStationDensity !== undefined &&
                                  step.currentStationDensity !== undefined
                                ) {
                                  const densityWarning =
                                    getDensityWarning(step);
                                  if (densityWarning) {
                                    return (
                                      <Badge
                                        variant="outline"
                                ...
                                return (
                                  ...
                                  >
                                    {formatTrafficLevel(traffic.level)}
                                  </Badge>
                                );
                              })()}
                          </div>
                          ...
                          <div className="flex gap-3 text-xs text-muted-foreground">
                            ...
                            {step.waitingTime && (
                              <>
                                <span>•</span>
                                <span>Wait: {step.waitingTime}s</span>
                              </>
                            )}
                          </div>
                        </div>
    ...
    <div className="h-[95vh] w-[430px] bg-background flex flex-col overflow-hidden">
      ...
      <header className="border-b bg-card flex-shrink-0">
        <div className="px-4 py-3 pb-2 flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-xl font-bold text-blue-600">Route Details</h1>
            <p className="text-xs text-muted-foreground">Review your journey</p>
          </div>
          {/* Route Summary Stats */}
            ...
            <div className="flex gap-2">
              {routes.map((route, idx) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`font-semibold ${isSelected ? "text-blue-600" : "text-foreground"}`}
                    >
                      Route {idx + 1}
                    </span>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                disabled
                value={
                  endLocation ? `${endLocation.name} (${endLocation.area})` : ""
                }
                className="disabled:opacity-100"
              />
            </div>
        ...
        <div className="flex-1 relative">
          ...
          {routeData && routeData.steps && consolidatedSteps.length > 0 && (
            <div className="absolute bottom-4 left-4 right-4 z-[1000]">
              <div className="space-y-1">
                {/* Thin Summary Card */}
                {currentStepIndex < consolidatedSteps.length &&
                  (() => {
                    const traffic = getStepTrafficLevel(currentStepIndex);
                    const bgColorMap = {
                      low: "bg-green-50/95",
                    ...
                    const thinCardBg = showEstablishmentsInMain
                      ? `${bgColorMap[traffic.level]} backdrop-blur ${
                          borderColorMap[traffic.level]
                        }`
                      : "bg-background/95 backdrop-blur border-blue-600/20";

                    return (
                      <Card className={`${thinCardBg} py-0`}>
                        <CardContent className="p-2 px-3">
                          <div className="flex items-center justify-between gap-2">
                                ...
                                <>
                                  <div
                                    className={`h-2 w-2 rounded-full ${
                                      dotColorMap[traffic.level]
                                    } animate-pulse flex-shrink-0`}
                                  />
                                  <div className="flex-1 min-w-0">
                                    <p
                                      className={`text-xs font-medium ${
                                        textColorMap[traffic.level]
                                      }`}
                                    >
                                      {isWaiting
                                        ? `Waiting for ${
                                    ...
                                    {(() => {
                                      ...
                                      const currentStep =
                                        consolidatedSteps[currentStepIndex];
                                      const stepModeInfo =
                                        getConsolidatedMode(currentStep);
                                      const isLRTMRT =
                                        stepModeInfo.vehicle.includes("LRT") ||
                                        stepModeInfo.vehicle.includes("MRT");

                                      // Debug logging
                                      console.log("[Density Check]", {
                                        stepName: currentStep.name,
                                        vehicle: stepModeInfo.vehicle,
                                        isLRTMRT,
                                        prevDensity:
                                      ...
                                      return (
                                        <p
                                          className={`text-xs ${
                                            textLightColorMap[traffic.level]
                                          } mt-0.5`}
                                        >
                                          Traffic:{" "}
                                          {formatTrafficLevel(traffic.level)}
                                ...
                                <>
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <p className="text-xs font-medium text-blue-600">
                                      {selectedPOIs.length} establishment
                                      {selectedPOIs.length !== 1 && "s"} nearby
                                    </p>
                                  </div>
                                </>
                              )}
                            </div>
                            {isWaiting && showEstablishmentsInMain && (
                              <Badge
                                className={`${
                                  badgeColorMap[traffic.level]
                            ...
                            <Button
                              ...
                              onClick={toggleEstablishmentsView}
                            >
                              {showEstablishmentsInMain ? (
                  ...
                  <CardContent className="p-4 pt-1">
                    {currentStepIndex === consolidatedSteps.length ? (
                      ...
                      <>
                        ...
                        {consolidatedSteps[currentStepIndex]?.mode !==
                          "BusTerminal" &&
                          (() => {
                            const currentStep =
                              consolidatedSteps[currentStepIndex];
                            const traffic =
                              getStepTrafficLevel(currentStepIndex);
                            ...
                            if (
                              ...
                              currentStep.previousStationDensity !==
                                undefined &&
                              currentStep.currentStationDensity !== undefined
                            ) {
                              const densityWarning =
                                getDensityWarning(currentStep);
                              if (densityWarning) {
                                  ...
                                  <div className="flex items-center gap-2 mb-3">
                                    <div
                                      className={`flex items-center gap-2 px-3 py-2 rounded-lg flex-1 ${
                                        densityWarning.level === "high"
                                          ? "bg-red-50"
                                          : densityWarning.level === "moderate"
                                            ? "bg-yellow-50"
                                            : "bg-green-50"
                                      }`}
                                    >
                        ...
                        <div className="flex items-start gap-4">
                          <div className="flex-shrink-0">
                            <div className="h-12 w-12 rounded-full bg-blue-600/10 flex items-center justify-center">
                              {(() => {
                                const mode = getConsolidatedMode(
                                  consolidatedSteps[currentStepIndex],
                                );
                                const Icon = mode.icon;
                                return (
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              ...
                              <Badge variant="outline">
                                {(
                                  consolidatedSteps[currentStepIndex].distance /
                                  1000
                                ).toFixed(1)}{" "}
                                km
                              </Badge>
                              <Badge variant="outline">
                                {Math.round(
                                  consolidatedSteps[currentStepIndex].duration /
                                    60,
                                )}{" "}
                                min
                              </Badge>
                                    ...
                                    >
                                      ...
                                      {densityWarning.waitForNext && (
                                        <span className="block mt-1 font-medium">
                                          💡 Recommendation: Wait for the train
                                          after next
                                        </span>
                                      )}
                                    </p>
