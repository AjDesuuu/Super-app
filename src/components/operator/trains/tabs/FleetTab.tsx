...
export default function FleetTab() {
  ...
  const [selectedStatus, setSelectedStatus] = useState<BusStatus | "all">(
    "all",
  );
  ...
  const filteredBuses = useMemo(() => {
    return mockBuses.filter((bus) => {
      ...
      const statusMatch =
        selectedStatus === "all" || bus.status === selectedStatus;
      ...
      for (const route of mockRoutes) {
        ...
        for (let i = 0; i < stops.length - 1; i++) {
          try {
            ...
            console.error(
              `Error fetching segment ${i} for route ${route.id}:`,
              error,
            );
      ...
      <div className="flex-1 relative">
        ...
        <div className="absolute top-4 left-4 bg-white rounded-lg shadow-md px-3 py-2 z-[1000]">
          <p className="text-xs text-gray-600">
            Showing <span className="font-bold">{filteredBuses.length}</span> of{" "}
            {mockBuses.length} buses
          </p>
        </div>
                    ...
                    <div
                      className={cn(
                        ...
                        selectedBus.passengers / selectedBus.capacity > 0.8
                          ? "bg-red-500"
                          : selectedBus.passengers / selectedBus.capacity > 0.6
                            ? "bg-yellow-500"
                            : "bg-green-500",
                      )}
                      style={{
                        width: `${(selectedBus.passengers / selectedBus.capacity) * 100}%`,
                      }}
                    />
                  </div>
                </div>
              ...
              {selectedBus.delayReason && (
                <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
                  ...
                  <p className="text-sm text-orange-900">
                    {selectedBus.delayReason === "traffic" &&
                      "🚦 Heavy Traffic"}
                    {selectedBus.delayReason === "breakdown" &&
                      "🔧 Mechanical Issue"}
                    {selectedBus.delayReason === "weather" &&
                      "🌧️ Weather Conditions"}
                    {selectedBus.delayReason === "passenger" &&
                      "👥 Passenger Delay"}
                    {selectedBus.delayReason === "accident" &&
                      "⚠️ Road Accident"}
                  </p>
                </div>
              )}
