...
const ALERT_TYPES = [
  {
    ...
    message:
      "Platform capacity exceeded 90%. Crowd density critical - entry control recommended.",
    actionLabel: "Activate WAIT Status",
    actionType: "wait_status" as const,
    icon: Radio,
    description:
      "This will broadcast a WAIT status to all passengers in the area via the NexPath app.",
  },
  {
    type: "dwell_time_overrun" as const,
    severity: "high" as const,
    message:
      "Train stopped for 60+ seconds past scheduled departure. Downstream delays expected.",
    actionLabel: "Adjust ETA Updates",
    actionType: "adjust_eta" as const,
    icon: Clock,
    description:
      "This will automatically update ETA windows for the next 5 downstream stations.",
  },
];

const DOWNSTREAM_STATIONS: Record<
  string,
...
const generateIncidentImpact = (alert: Alert) => {
  ...
  return {
    ...
    estimatedRecoveryTime:
      alert.severity === "critical" ? "45 mins" : "25 mins",
...
const getAlertTypeLabel = (type: Alert["type"]) => {
  return type === "platform_saturation"
    ? "PLATFORM SATURATION"
    : "DWELL TIME OVERRUN";
};
      ...
      <CardContent className="pt-4 space-y-3">
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-start gap-2">
            <div
              className={cn("p-1.5 rounded-full animate-pulse", config.iconBg)}
            >
