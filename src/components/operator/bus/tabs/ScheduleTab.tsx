  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      if (Math.random() < 0.3) {
        ...
        setTrips((prevTrips) => {
          const availableTrips = prevTrips.filter(
            (t) => t.status === "scheduled" && !tripTimers[t.id],
          );

          if (availableTrips.length === 0) return prevTrips;

          const randomTrip =
            availableTrips[Math.floor(Math.random() * availableTrips.length)];
      ...
      <div className="space-y-2">
        {filteredTrips.map((trip) => {
          ...
          const hasActiveTimer =
            timer > 0 &&
            ["arriving", "boarding", "delayed", "departed"].includes(
              trip.status,
            );
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start justify-between mb-2">
                ...
                <Badge
                  className={`${statusConfig.color} text-white text-xs shrink-0`}
                >
                  {statusConfig.label}
                </Badge>
              </div>
        ...
        <DialogContent className="max-w-[360px] max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              {activeTrip &&
                `${activeTrip.id} - ${formatTime(activeTrip.scheduled_time)}`}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2 py-3">
            {(Object.keys(tripStatusConfig) as TripStatus[]).map((status) => (
              <Button
                ...
                onClick={() =>
                  activeTrip && handleUpdateStatus(activeTrip, status)
                }
              >
                <Badge
                  className={`${tripStatusConfig[status].color} text-white mr-2 text-xs`}
                ></Badge>
                {tripStatusConfig[status].label}
              </Button>
            ))}
          ...
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              ...
              <span className="text-yellow-600">
                Will add 5-20 minutes delay and start countdown
              </span>
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2 py-3">
