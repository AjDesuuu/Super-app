...
const mockIncidents: Incident[] = [
  {
    id: "inc-001",
    type: "Delay",
    location: "Cubao Station",
    line: "LRT-2",
    severity: "medium",
    status: "active",
    time: "10:45 AM",
    description: "Signal system maintenance causing 5-minute delays",
  },
  {
    id: "inc-002",
    type: "Overcrowding",
    location: "Ayala Station",
    line: "MRT-3",
    severity: "high",
    status: "active",
    time: "11:20 AM",
    description: "Platform capacity at 95% during rush hour",
  },
  {
    id: "inc-003",
    type: "Technical",
    location: "Doroteo Jose",
    line: "LRT-1",
    severity: "low",
    status: "resolved",
    time: "09:15 AM",
    description: "Ticketing machine temporarily offline - resolved",
  },
];
...
export const generateCSV = (type: ReportType): string => {
  ...
  switch (type) {
    ...
    case "weekly":
      ...
      mockWeeklyData.forEach((day, i) => {
        const change =
          i > 0
            ? (
                ((day.passengers - mockWeeklyData[i - 1].passengers) /
                  mockWeeklyData[i - 1].passengers) *
                100
              ).toFixed(1)
            : "0.0";
    ...
    case "incidents":
      ...
      mockIncidents.forEach(
        (i) =>
          (csv += `"${i.id}","${i.type}","${i.location}","${i.line}","${i.severity}","${i.status}","${i.time}","${i.description}"\n`),
      );
      break;
  }
  return csv;
          ...
          ${mockWeeklyData
            .map((d, i) => {
              const chg =
                i > 0
                  ? (
                      ((d.passengers - mockWeeklyData[i - 1].passengers) /
                        mockWeeklyData[i - 1].passengers) *
                      100
                    ).toFixed(1)
                  : "0.0";
  ...
  return `
    ...
    <body>
      <div style="text-align: center; margin-bottom: 30px;">
        <h1 style="color: #000000;">NexPath Rail Operations</h1>
      ...
      <div style="margin-top: 40px; border-top: 1px solid #e5e7eb; text-align: center; color: #6b7280; font-size: 12px;">
        <p>© 2026 NexPath. All rights reserved.</p>
      </div>
    </body></html>`;
};
...
export const handleExportLogic = async (
  type: ReportType,
  format: ExportFormat,
) => {
  ...
  if (format === "csv") {
    ...
    downloadFile(csv, `nexpath-${type}-${timestamp}.csv`, "text/csv");
  } else {
    const html = generatePDF(type);
    downloadFile(html, `nexpath-${type}-${timestamp}.html`, "text/html");
  }
};
